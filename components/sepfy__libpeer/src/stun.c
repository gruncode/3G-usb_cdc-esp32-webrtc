  ...
  while (pos < length) {
    ...
    switch (ntohs(attr->type)) {
      ...
      case STUN_ATTR_TYPE_NETWORK_COST:
        // Do nothing
        break;
      case STUN_ATTR_TYPE_ERROR_CODE: // 0x0009
        {
          // Parse ERROR-CODE attribute (RFC 5389 Section 15.6)
          uint8_t class_val = attr->value[2];
          uint8_t number = attr->value[3];
          uint16_t error_code = class_val * 100 + number;
          
          // If there's a reason phrase, extract it
          char reason[128] = {0};
          uint16_t reason_length = ntohs(attr->length) - 4; // 4 bytes for header
          
          if (reason_length > 0 && reason_length < sizeof(reason)) {
            memcpy(reason, attr->value + 4, reason_length);
            reason[reason_length] = '\0';
          } else {
            strcpy(reason, "No reason provided");
          }
        ...
        break;
      default:
        // Check if it's a vendor-specific attribute (high bit set)
        if ((ntohs(attr->type) & 0x8000) == 0x8000) {
          LOGI("Vendor-specific Attribute Type: 0x%04x (length: %d)", 
               ntohs(attr->type), ntohs(attr->length));
        } else {
